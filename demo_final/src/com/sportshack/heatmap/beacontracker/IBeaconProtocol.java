...
public class IBeaconProtocol {
	 ...
	 */		
	public static int SCANNING_PERIOD = 1100;
	...
	private BluetoothAdapter.LeScanCallback mLeScanCallback = new BluetoothAdapter.LeScanCallback() {
	    ...
	    public void onLeScan(final BluetoothDevice device, int rssi,
	    	...
	    	if(_arrOrderedIBeacons.contains(newBeacon)){
	    		float newDistance = (float)calculateDistance(newBeacon.getPowerValue(), rssi);
	    		IBeacon previousIBeaconInfo = findIfExists(newBeacon);
	    		float oldDistance = previousIBeaconInfo.getProximity();
	    		if(newDistance < oldDistance){
	    			Log.i(Utils.LOG_TAG,"Updating distance");
	    			previousIBeaconInfo.setProximity(newDistance);
	    			previousIBeaconInfo.set_rssi(rssi);
		    		// Sort again
	    	...
	    	newBeacon.setProximity((float)calculateDistance(newBeacon.getPowerValue(), rssi));
	...
	private class IBeaconProximityComparator implements Comparator<IBeacon> {
	    ...
	    public int compare(IBeacon b1, IBeacon b2) {
	    		return Float.compare(b1.getProximity(), b2.getProximity());
	    }
	}
